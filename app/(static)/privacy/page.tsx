...
export const metadata: Metadata = {
  title: "개인정보 처리방침",
  description: `${siteConfig.name}의 계정, 회의 음성, 스크립트 데이터와 서비스 이용 기록 처리에 관한 안내입니다.`,
...
const sections = [
  {
    ...
    body: [
      ...
      "회의 기록 기능을 사용하는 동안 마이크에서 입력된 음성 데이터와 그 음성으로 생성된 실시간 스크립트, 확정 문장, 화자 구분 결과, 노트 제목 및 회의 메타데이터를 처리합니다.",
  ...
  },
  {
    title: "2. 처리 목적",
    body: [
      "회원 식별과 로그인 유지, 워크스페이스 운영, 실시간 회의 스크립트, 노트 저장과 조회, 서비스 보안 및 부정 이용 방지를 위해 정보를 사용합니다.",
  ...
  {
    title: "3. 음성 및 스크립트 데이터",
    body: [
      "마이크는 이용자가 기록 시작을 선택하고 브라우저 권한을 허용한 경우에만 사용됩니다. 실시간 음성 데이터는 스크립트를 만들기 위해 음성 인식 사업자에게 전송되며, 받아 적은 문장이 화면에 표시됩니다.",
      "회의 중 전송된 음성은 스크립트 정확도 개선과 화자 구분, 장애 복구를 위해 암호화된 저장소에 보관됩니다. 이 저장소는 대한민국(서울) 리전에 있고 외부에 공개되지 않습니다.",
      ...
      "자동 스크립트와 화자 구분에는 오류가 포함될 수 있으므로 중요한 결정이나 고유명사는 이용자가 직접 확인해야 합니다.",
    ],
  },
  {
    title: "4. AI 처리와 외부 도구",
    body: [
      "요약, 실시간 정리, 질의 응답을 만들기 위해 해당 회의의 스크립트와 노트, 이용자의 질문이 대규모 언어 모델 제공자에게 전송됩니다. 답변에는 실제로 참고한 회의록이 함께 표시됩니다.",
      "검색과 참조를 위해 스크립트와 노트에서 벡터 표현을 생성해 보관할 수 있습니다.",
  ...
  },
  {
    ...
    body: [
      ...
      "분석 도구는 운영 환경에서만 동작하며, 회의 음성·스크립트·노트의 내용은 분석 도구로 전송하지 않습니다.",
    ],
  },
  {
    title: "6. 보관 및 파기",
    body: [
      ...
      "노트를 삭제하면 노트와 스크립트, 요약이 삭제됩니다. 저장된 음성 파일의 파기는 별도 절차로 이루어지며, 그 주기는 현재 확인 중입니다. 확인되는 대로 본 방침에 반영합니다.",
  ...
  },
  {
    ...
    body: [
      ...
      "실시간 음성 인식은 Soniox 를 사용합니다. 회의 중 마이크로 입력된 음성 데이터가 스크립트 처리를 위해 해당 사업자에게 전송됩니다.",
      "화자 분리는 pyannote.ai 를 사용합니다. 회의가 끝난 뒤 저장된 음성 파일에 접근할 수 있는 한시적 링크를 전달하고, 해당 사업자가 그 파일을 내려받아 발화자를 구분합니다.",
      "회의 내용 요약, 결정·업무 추출, 에이전트 채팅 응답 생성과 검색용 벡터 생성에는 OpenAI 를 사용합니다. 이때 받아 적은 텍스트와 회의 메타데이터, 이용자의 질문이 해당 사업자에게 전송됩니다. 모델 제공자는 운영 상황에 따라 달라질 수 있고, 바뀌면 이 처리방침을 고쳐 알립니다.",
      "인프라는 Amazon Web Services(서울 리전)를 사용하며, 서비스 상태 관측에는 Grafana Cloud 를 사용합니다.",
      "웹 서비스 호스팅은 Vercel 을 사용합니다. 페이지를 서버에서 먼저 그리는 구조라, 열람 중인 노트와 스크립트가 그 런타임을 거쳐 화면으로 전달됩니다.",
      "접속 분석은 Vercel 을, 이용 통계 분석은 Google 을 사용하며 접속 기록과 기기·브라우저 정보를 전송합니다. 회의 음성·스크립트·노트는 이 분석 도구로 전송하지 않습니다.",
      "위 사업자 가운데 음성 인식(Soniox), 화자 분리(pyannote.ai), LLM 분석(OpenAI), 호스팅·접속 분석(Vercel · Google) 사업자는 국외에서 처리합니다. 전송되는 항목은 각각 회의 음성 데이터, 회의 음성 파일, 스크립트 텍스트와 회의 메타데이터, 열람 중인 노트·스크립트와 접속 기록이며, 전송 목적은 위에 적은 처리 수행에 한정됩니다. 전송 시기는 해당 기능을 이용하는 시점이고 방법은 암호화된 네트워크 전송입니다.",
...
export default function PrivacyPage() {
  return (
    <LegalDocument
      label="법적 고지"
      title="개인정보 처리방침"
      description="회의를 기록하는 순간부터 저장된 스크립트를 다시 확인할 때까지, HeyMoa가 어떤 정보를 왜 처리하는지 안내합니다."
