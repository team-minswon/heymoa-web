 ...
 * 화면이 그 자리에 굳어 다시 볼 방법이 없었다.
 *
 * 두 가지만 반복을 미룬다. **화면 밖이면 아예 안 돈다**(대본 자체가 쉰다), 그리고 **손수
 * 보낸 답이 흐르는 동안에는 예약하지 않는다** — 남은 대기가 답보다 짧아서 예약해 두면
 * 답이 중간에 지워진다.
 */
...
export const TRANSCRIPT: Line[] = [
  ...
  },
  {
    at: "00:44",
    who: "김민서",
    text: "에이전트가 근거를 붙여 뒀어요. 오른쪽 정리에서 결정 항목을 펼치면 됩니다.",
  },
  {
    at: "01:02",
    who: "정우재",
  ...
  },
  {
    at: "01:33",
    who: "이서연",
    text: "그 이슈에 이 회의 결정을 근거로 같이 붙여 주세요. 다음에 들어올 사람도 볼 수 있게요.",
  },
  {
    at: "01:48",
    who: "김민서",
    text: "네. 승인 화면에서 확인하고 내보내겠습니다. 오늘 남길 건 여기까지입니다.",
  },
];
...
export const SUMMARY: Array<[string, Array<[string, string]>]> = [
  [
    ...
    [
      [
        "온보딩 이탈을 이번 스프린트의 첫 기준선으로 잡고, 결제 화면 개편은 뒤로 미뤘습니다.",
        "00:00",
      ],
    ],
  ],
...
export const CONTEXT: Item[] = [
  ...
  {
    kind: "결정",
    title: "온보딩 이탈 지표를 이번 주 기준선으로 삼는다",
    at: "00:52",
    more: 2,
    meta: "결정 · 수정 1",
    metaSm: "수정 1",
...
/** 한 바퀴가 끝나고 처음으로 돌아가기까지 쉬는 시간. 끝 화면이 한 박자 머물 만큼만. */
const LOOP_GAP = 1500;
...
const HOLD: Record<Beat["t"], number> = {
  ...
  note: 780,
  ask: 1500,
  // **여기만 느리다.** 이 순간이 회의를 끝내는 장면이고, 다른 대목과 같은 속도로 지나가면
  // 「버튼이 잠깐 반짝했다」로만 남는다. 손이 닿고 → 눌러 들어가고 → 머물고 → 떼는 네
  // 동작이 다 보일 만큼 준다(CSS `lp-press`가 같은 1.4초를 쓴다).
  press: 1400,
  // 칩이 바뀌고 버튼이 자리를 접는 것까지 보고 나서 요약으로 넘어간다.
  end: 1300,
  summary: 640,
};
...
export type Demo = {
  ...
  turns: Ask[];
  /**
   * 흐르는 중인 답의 진행값. **어느 turn의 것인지는 `typingAt`이 말한다** — 배열의
   * 마지막이라고 가정하면 안 된다. 방문자가 대본보다 먼저 물어보면 손으로 보낸 것이
   * 배열 끝에 있고 대본의 질의는 그 앞에 선다.
   */
  typing: number | null;
  /** `typing`이 붙는 turn의 자리. 흐르는 것이 없으면 -1. */
  typingAt: number;
  ask: (item: Ask) => void;
...
export function useDemo({
  ...
  /**
   * **누른다고 멈추지 않는다.** 멈추는 것은 탭 이동뿐이고 그건 아래 `noteTab`·`railTab`이
   * 판다. 대신 **화면 밖이면 쉰다** — 안 보이는 동안 16ms 간격으로 상태를 갱신하며 큰
   * 트리를 두 벌 다시 그릴 이유가 없다.
   */
  const playing = visible && !skip && !done;

  /**
   * 손으로 보낸 답이 흐르는 중인가. **글자 수가 아니라 참·거짓이다** — 진행값을 그대로
   * 아래 타이머 효과의 의존성에 넣었더니 16ms마다 정리 함수가 돌아 **대본 타이머가 계속
   * 취소됐다.** 답이 흐르는 동안 전사와 사건까지 같이 멈췄다.
   */
   ...
   */
  useEffect(() => {
    // **손수 보낸 답이 흐르는 동안에는 예약하지 않는다.** 남은 대기(1.5초)가 「생각 0.62초
    // + 타이핑」보다 짧아서, 예약해 두면 답이 중간에 지워진다. 답이 끝나면 이 효과가 다시
    // 돌아 그때부터 다시 잰다.
    if (!visible || skip || !done || manualBusy) return;
  ...
  }, [playing, cursor, progress, manualBusy]);
   ...
   * `ask` 대목의 *진행*이지 *진입*이 아니다 — 앞 대목(`rail`)의 예약이 이미 터져 있으면
   * 커서는 `ask`에 들어온 채로 멈춘다. 그대로 내보내면 그 turn을 가리키는 진행값이 없어
   * 대본 답이 **완성본으로** 떴다가, 손수 답이 끝나는 순간 「생각하는 중」으로 되감긴다.
   */
  const scriptAsk = manualBusy ? null : view.scriptAsk;
  ...
  return {
    ...
    // 손으로 보낸 것은 배열 끝, 대본의 질의는 `SEED` + 지난 대본 질의 바로 뒤다.
    typingAt:
      manualTyping !== null
        ? 1 +
          view.scriptTurns.length +
          (scriptAsk ? 1 : 0) +
          extra.length -
          1
        : scriptAsk
          ? 1 + view.scriptTurns.length
          : -1,
    ask,
    pinRail,
  };
