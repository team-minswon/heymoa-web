...
import { ProposalRail, formatFreshness } from "@/components/notes/proposal-rail";
...
function renderRail(
  ...
  const state: ContextState = {
    ...initialContextState,
    proposals: Object.fromEntries(proposals.map((c) => [c.proposalId, c])),
    ...over,
  };
  realtime.value = {
    context: {
  ...
  render(<ProposalRail onEvidenceSelect={onEvidenceSelect} />);
  return { onEvidenceSelect, onRetry };
}

afterEach(cleanup);

describe("ProposalRail", () => {
  ...
  it("개수가 목록에 보이는 카드 수와 같고 완결을 주장하지 않는다", () => {
    renderRail([
      ...
      head({ proposalId: "0HZX2K7M9Q4A2", createdSequence: 20 }),
    ]);
  ...
  it("배치와 후보 event가 어떤 처리로 이어졌는지 시각화한다", () => {
    renderRail([head({ proposalId: "0HZX2K7M9Q4A1", operation: "CORRECT" })], {
      activities: [
        {
          type: "proposal",
    ...
    expect(within(flow).queryByText("정리 반영")).not.toBeInTheDocument();

    fireEvent.click(within(flow).getByRole("button", { name: "처리 내역" }));

    expect(within(flow).getByText("정리 반영")).toBeInTheDocument();
  ...
  it("재동기화 후 복구되면 과거 경고 대신 최신 정상 상태를 보인다", () => {
    renderRail([head({ proposalId: "0HZX2K7M9Q4A1" })], {
      activities: [
  ...
  it("묶음 머리로 유형별로 접고, 접어도 개수는 남는다", () => {
    renderRail([
      head({ proposalId: "0HZX2K7M9Q4A1", kind: "DECISION" }),
      head({
        proposalId: "0HZX2K7M9Q4A2",
        kind: "ACTION_ITEM",
  ...
  it("근거가 없는 후보는 펼칠 것이 없으므로 비활성이다", () => {
    renderRail([head({ proposalId: "0HZX2K7M9Q4A1", citations: [] })]);
    const claim = screen.getByRole("button", { name: /MongoDB를 사용한다/ });
