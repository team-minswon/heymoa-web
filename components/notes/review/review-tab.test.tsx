 ...
 */
const server = setupServer(...meetingFlowHandlers, ...projectTaskHandlers, ...restHandlers);
...
const rowBox = async (content: string) => (await row(content)).closest("[data-item-id]") as HTMLElement;

describe("검토 가능한 회의", () => {
  it("개요와 네 섹션을 그리고, 주제 칩으로 그 주제의 항목만 남긴다", async () => {
    renderTab();

    expect(await screen.findByText(/문제 정의를 「회의 뒤 할 일이 확정되지 않는다」로 좁히고/)).toBeInTheDocument();
    ...
    expect(await row("요금은 좌석이 아니라 회의 시간 기준으로 계산한다")).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "설문 근거는 출처와 표본 수를 발표 자료에 함께 적는다" })
    ).not.toBeInTheDocument();
    expect(screen.getByRole("status")).toHaveTextContent("주제 02차별점과 요금");

    fireEvent.click(screen.getByRole("button", { name: "전체 보기" }));
    expect(await row("설문 근거는 출처와 표본 수를 발표 자료에 함께 적는다")).toBeInTheDocument();
  });
  ...
  it("주제 목차는 번호 · 제목 · 한 줄 서술 · 항목 수를 세우고, 줄에는 주제 이름이 붙는다", async () => {
    ...
    expect(within(index).getByRole("button", { name: /01\s*회의가 끝난 뒤 할 일이 흐려지는 문제/ })).toHaveTextContent(
      "회의 뒤 할 일이 흐려지는 문제를 한 문장으로 좁히고"
    );
    expect(within(index).getAllByRole("button")).toHaveLength(3);

    fireEvent.click(screen.getByRole("button", { name: "주제 5개 더" }));
    expect(await screen.findByRole("button", { name: /08\s*다음 멘토링과 활동비/ })).toBeInTheDocument();
    expect(screen.getAllByTitle("주제 01 · 회의가 끝난 뒤 할 일이 흐려지는 문제").length).toBeGreaterThan(0);
  });
  ...
  it("수정하고, 제외했다가 제외를 취소한다", async () => {
    ...
    expect(await row("설문 근거는 출처 · 표본 수 · 조사 연도를 함께 적는다")).toBeInTheDocument();
    ...
    await waitFor(() => expect(screen.queryByText(/제외됨/)).not.toBeInTheDocument());
  });
  ...
  it("고를 제안을 다 고르기 전에는 확정을 막고, 이전 결정을 끝내기로 고르면 확정 줄이 바뀐 뒤 확정한다", async () => {
    ...
    expect(await screen.findByText("고르지 않은 제안이 3개 남았습니다")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "검토 완료" })).toBeDisabled();

    // 제안은 줄을 펼치지 않아도 줄 아래에 서 있다.
    const box = await rowBox("첫 화면 메시지는 회의 기록보다 회의 뒤 실행 연결로 둔다");
    fireEvent.click(within(box).getByRole("radio", { name: /끝내기/ }));
    // 선택은 검토본에 저장된 뒤에 선다.
    await waitFor(() => expect(screen.getByText(/개 끝남/)).toHaveTextContent("이전 결정 1개 끝남"));
    expect(within(box).getByRole("radio", { name: /끝내기/ })).toHaveAttribute("aria-checked", "true");

    for (const content of ["경쟁 서비스 요금제를 한 표로 정리한다", "구현 결과를 기대 효과 순서로 다시 배치한다"]) {
      const toggle = within(await rowBox(content)).getByRole("radiogroup", { name: "기존 할 일에 반영할지" });
      await waitFor(() => expect(within(toggle).getByRole("radio", { name: "유지" })).toBeEnabled());
      fireEvent.click(within(toggle).getByRole("radio", { name: "유지" }));
      await waitFor(() => expect(within(toggle).getByRole("radio", { name: "유지" })).toHaveAttribute("aria-checked", "true"));
    }
    await waitFor(() => expect(screen.getByRole("button", { name: "검토 완료" })).toBeEnabled());
  ...
  it("제안의 선택은 저장돼 다시 열어도 같게 보이고, 끝낼 이전 결정의 내용과 출처가 선다", async () => {
    renderTab();
    const box = await rowBox("첫 화면 메시지는 회의 기록보다 회의 뒤 실행 연결로 둔다");
    expect(within(box).getByText(/로드맵 브레인스토밍 · .* 확정/)).toBeInTheDocument();
    fireEvent.click(within(box).getByRole("radio", { name: /끝내기/ }));
    await waitFor(() => expect(within(box).getByRole("radio", { name: /끝내기/ })).toHaveAttribute("aria-checked", "true"));

    cleanup();
    renderTab();
    const again = await rowBox("첫 화면 메시지는 회의 기록보다 회의 뒤 실행 연결로 둔다");
    expect(within(again).getByRole("radio", { name: /끝내기/ })).toHaveAttribute("aria-checked", "true");
  });
  ...
  it("이슈 · 질문에도 담당 · 기한 칸이 서고, 회의 중에 풀린 것은 해결됨으로 선다", async () => {
    ...
    expect(within(open).getAllByRole("button", { name: "기한 정하기" }).length).toBeGreaterThan(0);

    fireEvent.click(within(open).getByRole("button", { name: /이슈 · 질문 \d+개 더/ }));
    expect((await within(open).findAllByText("해결됨")).length).toBeGreaterThan(0);
  });

  it("기존 할 일 변경을 반영하면 그 할 일에 바로 저장된다", async () => {
    renderTab();
    const toggle = within(await rowBox("구현 결과를 기대 효과 순서로 다시 배치한다")).getByRole("radiogroup", {
      name: "기존 할 일에 반영할지",
    });
    await waitFor(() => expect(within(toggle).getByRole("radio", { name: "반영" })).toBeEnabled());
    fireEvent.click(within(toggle).getByRole("radio", { name: "반영" }));

    expect(await within(toggle).findByRole("radio", { name: /반영됨/ })).toHaveAttribute("aria-checked", "true");
    ...
    const task = data.tasks.find((row: { taskId: string }) => row.taskId === projectTasks.idOf("results-slide"));
  ...
  it("요약이 오는 동안 그래프 보기는 없다고 하지 않고 자리만 잡는다", async () => {
    ...
    expect(await screen.findByLabelText("그래프 불러오는 중")).toBeInTheDocument();
    expect(screen.queryByText("주제 묶음이 없어 그래프를 그릴 수 없습니다")).not.toBeInTheDocument();
  });

  it("요약 조회가 실패하면 요약이 없다고 하지 않고 다시 불러올 길을 둔다", async () => {
    server.use(
      http.get("*/v1/notes/:noteId/meeting-review/summary", () =>
        HttpResponse.json({ success: false, data: null, error: { code: "INTERNAL", message: "오류" } }, { status: 500 })
      )
    );
    renderTab();

    expect(await screen.findByText("요약을 불러오지 못했습니다.")).toBeInTheDocument();
    expect(screen.queryByText(/주제 요약이 없습니다/)).not.toBeInTheDocument();
    expect(within(section("개요")).getByRole("button", { name: "다시 시도" })).toBeInTheDocument();
  });
  ...
  it("담당으로 고를 사람 목록을 못 읽으면 빈 목록처럼 두지 않고 다시 불러올 길을 둔다", async () => {
    ...
    expect(await screen.findByText("담당으로 고를 사람 목록을 불러오지 못했습니다.")).toBeInTheDocument();
  });
  ...
  it("할 일에 반영한 뒤 선택 저장이 실패하면 알리고, 다시 누르면 선택만 저장한다", async () => {
    ...
    fireEvent.click(within(toggle).getByRole("radio", { name: "반영" }));
    expect(await screen.findByText(/할 일에는 반영했지만 선택을 저장하지 못했습니다/)).toBeInTheDocument();

    fireEvent.click(within(toggle).getByRole("radio", { name: "반영" }));
    expect(await within(toggle).findByRole("radio", { name: /반영됨/ })).toHaveAttribute("aria-checked", "true");
    expect(screen.queryByText(/할 일에는 반영했지만 선택을 저장하지 못했습니다/)).not.toBeInTheDocument();
  });
  ...
  it("다른 사람이 먼저 수정했으면 그 줄에 최신 내용을 불러왔다고 알린다", async () => {
    ...
    const review = await (await fetch(`http://localhost/v1/notes/${MENTORING_NOTE_ID}/meeting-review`)).json();
    const other = review.data.items[0];
    await fetch(`http://localhost/v1/notes/${MENTORING_NOTE_ID}/meeting-review/items/${other.itemId}`, {
      method: "PATCH",
    ...
    expect(await screen.findByText(/다른 사람이 먼저 수정해 최신 내용을 불러왔습니다/)).toBeInTheDocument();
  });
  ...
  it("항목을 추가하는 사이 다른 사람이 먼저 수정했으면 쓴 내용을 남긴 채 알린다", async () => {
    ...
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith(CONFLICT_MESSAGE));
    expect(screen.getByRole("textbox", { name: "새 항목 내용" })).toHaveValue("발표는 10분 안에 끝낸다");
  });
  ...
  it("그래프 보기에서 항목을 고르면 그 아래에 수정 기록이 선다", async () => {
    ...
    fireEvent.click(screen.getByRole("radio", { name: "그래프" }));
    const node = (await screen.findAllByRole("button", { name: /무료 구간은 월 5시간으로 두고 팀 요금제에서는 뺀다/ }))[0];
  ...
  it("회의 시작자가 아니면 읽기만 한다", async () => {
    renderTab(MENTORING_NOTE_ID, { isStarter: false });
    fireEvent.click(await row("설문 근거는 출처와 표본 수를 발표 자료에 함께 적는다"));

    expect(screen.queryByRole("button", { name: "결정 추가" })).not.toBeInTheDocument();
...
describe("검토본이 서기 전과 뒤", () => {
  ...
  it("지난 노트는 구형 요약을 읽기만 한다", async () => {
    ...
    expect(screen.queryByRole("button", { name: "요약 다시 만들기" })).not.toBeInTheDocument();
  });
  ...
  it("회의가 끝나기 전에는 요약이 언제 생기는지만 말한다", () => {
    ...
    expect(screen.getByText("요약은 회의가 끝나면 정리됩니다")).toBeInTheDocument();
  });
});
