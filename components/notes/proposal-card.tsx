...
import type { ContextCard } from "@/lib/notes/proposals/reducer";
import {
  CONTEXT_KIND_ICON,
  CONTEXT_KIND_LABEL,
...
function statusNote(proposal: ProposalHead) {
  if (proposal.closeReason === "RETRACTED") return "철회됨";
  if (proposal.closeReason === "RESOLVED") return "답변됨";
  if (proposal.kind === "QUESTION") return "답 대기";
  return null;
}
 ...
 */
function EvidenceTimes({ citations }: { citations: ProposalCitation[] }) {
...
function ProposalBody({
  ...
  const evidenceId = `context-citations-${proposal.proposalId}`;
  const hasEvidence = proposal.citations.length > 0;
  const retracted = proposal.closeReason === "RETRACTED";
  const note = statusNote(proposal);
  ...
  const metaParts: React.ReactNode[] = [
    ...
    proposal.operation === "CREATE" ? null : (
      <span
        key="operation"
        className={cn(
      ...
      >
        {CONTEXT_OPERATION_LABEL[proposal.operation]}
      </span>
    ),
    note ? (
    ...
    proposal.aiSemanticRevisionCount > 0 ? (
      <span
        key="revisions"
        className="text-[11.5px] tabular-nums text-[var(--el-muted-soft)]"
      >
        {`수정 ${proposal.aiSemanticRevisionCount}`}
      </span>
    ) : null,
  ].filter(Boolean);
    ...
    <div className="flex flex-col gap-[7px]">
      ...
      >
        ...
        <span className="flex shrink-0 items-center gap-2 pt-[3px]">
          <EvidenceTimes citations={proposal.citations} />
          {hasEvidence ? (
            <>
              <span className="sr-only">{` 근거 ${proposal.citations.length}개`}</span>
              <ChevronDown
                aria-hidden
                className={cn(
      ...
      <AnimatePresence initial={false}>
        {open && hasEvidence ? (
          ...
          >
            ...
            >
              {proposal.citations.map((citations) => (
                <li key={`${citations.segmentId}-${citations.role}`}>
                  {/* **`segmentId` 로만 찾는다** — `startedAtMs` 는 세션별 오프셋이라 세션이
                      둘 이상이면 화면의 시각과 어긋난다(APP-398 선례). */}
                  <button
                    type="button"
                    onClick={() => onEvidenceSelect(citations.segmentId)}
                  ...
                  >
                    ...
                    <time className="shrink-0 text-[12px] tabular-nums text-[var(--el-muted-soft)] transition-colors group-hover:text-[var(--el-muted)]">
                      {formatOffset(citations.startedAtMs)}
                    </time>
                  </button>
                </li>
              ))}
