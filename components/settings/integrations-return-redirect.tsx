 ...
 * 설정은 모달이라 전용 라우트가 없으므로, 기본 워크스페이스로 리다이렉트하며 쿼리를 그대로 넘긴다 —
 * 그러면 workspace-app-shell이 provider·status를 읽어 연동 모달을 열고 결과 토스트를 띄운다(APP-194).
 * 주소에 workspaceId가 없어 기본 워크스페이스로 연다(연동 자체는 서버가 올바른 워크스페이스에 저장; 모달은 표시용).
 */
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        router.replace(selected ? `/w/${selected.workspaceId}${suffix}` : "/");
      } catch {
