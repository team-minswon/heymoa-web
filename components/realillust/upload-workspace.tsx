    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
      <div className="rounded-2xl bg-[var(--cg-house)] p-6 text-white shadow-[0_16px_40px_rgba(30,57,50,0.18)] sm:p-8">
        <div className="flex min-h-[420px] flex-col justify-between gap-10">
          <div>
            <p className="text-sm font-semibold text-[var(--cg-gold)]">
              {siteConfig.name}
            </p>
            <h2 className="mt-4 max-w-3xl text-4xl font-semibold leading-tight tracking-[-0.01em] sm:text-5xl">
              AI 일러스트 검사부터 공모전 출품 이미지 검토까지 한 화면에서
              확인합니다.
            </h2>
            <p className="mt-5 max-w-2xl text-lg leading-8 text-white/78">
              AI 생성 일러스트, 디자인 시안, 공모전 제출 이미지의 메타데이터와
              의심 영역 신호를 조합해 빠르게 판별하는 검사 서비스입니다.
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-4">
            {steps.map((step, index) => (
              <div
                key={step}
                className="rounded-xl border border-white/14 bg-white/8 p-4"
              >
                <div className="text-sm font-semibold text-[var(--cg-gold)]">
                  0{index + 1}
                </div>
      ...
      <div className="rounded-2xl bg-white p-5 shadow-[0_0_0_1px_rgba(0,0,0,0.06),0_2px_8px_rgba(0,0,0,0.1)]">
        ...
        >
          ...
          <span className="mt-2 max-w-sm text-sm leading-6 text-black/58">
            JPEG, PNG, WebP. MVP 제한은 10MB이며 원본 이미지는 기본 24시간 후
            삭제됩니다.
          </span>
          <input
            id="image-upload"
