...
**무엇을 기다리는지**로 가릅니다. "로딩"은 한 가지가 아닙니다.
...
작업(수 분)에 **같은 skeleton + 같은 문구**를 썼습니다 — 「다른 화면으로 옮겨도 됩니다」가
200ms 조회에 붙으면 거짓말입니다.

## skeleton은 데이터만 가립니다

**라벨·섹션 머리글·정적 문구는 도착을 기다리는 것이 아닙니다.** 회색 막대로 덮지 말고
...
**한 덩어리로 여러 줄을 대신하지 않습니다.** `h-40` 하나가 표 5줄 자리에 서면 도착할 때
높이가 두 배가 됩니다. 실제 행 구조를 그대로 쓰고 값만 막대로 바꾸는 것이 가장 안전합니다 —
같은 wrapper 컴포넌트를 재사용하면 나중에 실제 화면만 고쳐도 어긋나지 않습니다.
...
같은 이유로 **조건부로 나타나는 칩·배지가 옆 요소를 밀지 않게** 자리를 미리 잡습니다.
