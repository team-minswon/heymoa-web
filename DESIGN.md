...
- **푸터** — 아트보드의 어두운 파도 띠를 공용 `Footer`에 옮겨 그렸다. **뜨는 곳을 경로로
  가른다**(`MARKETING_PATHS`): `/` · `/terms` · `/privacy`. `FooterGate`가 루트 레이아웃에
  있어서 `/invite` · `/mock-oauth` · `/settings/integrations`도 이 컴포넌트로 오는데, 그쪽은
  제품 흐름이라 예전 범용 푸터를 쓴다 — 안 가르면 랜딩 전용 파도가 거기까지 번진다.
  「축약 판」(`simplified`)은 없앴다: 그것만 제품 면 회색(`--el-canvas`)이라 크림 문서
  아래에 회색 띠가 남았다.
- **「왜 만드나」 셋째 줄** — 아직 없는 동작이라 라벨을 「방향」으로 적고 나침반 아이콘을
  쓴다. **상자 모양까지 가르지는 않는다** — 점선 테두리로 갈라 그렸다가 줄 하나만 미완성처럼
  보여 되돌렸다.

**약관·개인정보도 같은 면이다.** 이 문서 첫 줄이 「마케팅 면(랜딩·약관)」을 한 묶음으로
정하므로, 두 문서 페이지도 크림 캔버스와 `--lp-*`를 쓴다(`legal-document.tsx`). 예전에는
제품 면 토큰에 그라데이션 오브와 유리 패널이었는데, 랜딩이 크림 편집 조판으로 옮겨간 뒤로는
같은 사이트의 두 페이지가 서로 다른 제품처럼 보였다.

- eyebrow는 갈래(「법적 고지」)를 적는다. 제목이 이미 문서 이름이라 거기 「Terms」를 다시
  적으면 같은 말이 두 번이다
- 목차는 크림 카드, 본문은 흰 카드 — 「옮겨 다니는 곳」과 「읽는 곳」이 색으로 갈린다
- 절 머리는 세리프다(랜딩 요약 절과 같은 결). 본문은 14.5/1.85로 랜딩 리드(16/1.75)보다
  잘게 — 처음부터 끝까지 읽는 글이라 줄이 짧아야 한다
- **스크롤 리빌을 안 건다.** 랜딩 밴드는 훑어보는 자리라 하나씩 떠도 되지만, 읽는 동안
  문단이 나타나면 방해다
- `(static)`의 `PageTransition`도 걷어냈다. 랜딩과 같은 이유다 — `opacity:0`인 HTML이
  나가서 JS를 끄면 문서가 통째로 안 보였다
...
- **손대면 탭만 고정한다.** 누르면 그 기둥의 탭이 그 자리에 못 박히고, **대본은 계속
  ...
  레일까지 멈추지 않는다.
- **장면이 바뀌는 이동은 고정을 이긴다.** 대본이 탭을 옮기는 자리는 둘뿐이고 둘 다 새
  장면의 시작이다 — 레일이 「내 에이전트」로 가는 것(질의)과, 회의가 끝나 요약 탭으로 가는
  것. 고정이 이것까지 막으면 방문자가 **아무거나 한 번 눌렀다는 이유로 장면 하나가 통째로
  안 보인다**(실제로 그랬다). 고정이 지키는 것은 「방금 누른 것이 곧바로 되돌려지지 않는
  다」까지이고, 다음 장면에는 같이 간다. 요약 탭으로 가는 이동에는 근거가 하나 더 있다 —
  회의가 끝나면 **앱이** 그렇게 한다(`meeting-controls.tsx`의 `onMeetingEnded`).
- **「회의 종료」는 그림이다.** 한때 진짜로 눌렸는데, 누르는 순간 기록 중이던 회의가
  종료로 확 넘어가서 「내가 뭘 부순 건가」로 읽혔다. 앱에서는 다이얼로그가 한 번 더 묻고
  되돌릴 수 없는 일이라는 것을 말해 주지만(`meeting-end-dialog.tsx`), 랜딩에서 그 확인창
  까지 그리면 이 자리가 회의 종료를 배우는 화면이 되어 버린다. 회의를 끝내는 것은 대본이다.
- **누르는 순간과 끝나는 순간을 다른 대목으로 나눈다.** 한 대목이면 버튼이 사라지고 칩이
  바뀌는 것만 남아서, 정작 「누가 눌렀다」는 순간이 화면 어디에도 없다. **이 대목만 느리다**
  (1.4초) — 회의를 끝내는 장면이라 다른 대목과 같은 속도로 지나가면 「버튼이 잠깐 반짝했다」
  로만 남는다. 손이 닿고(0.25초, 색이 든다) → 눌러 들어가고(0.42초) → 머물고(0.78초까지) →
  떼는(1.0초) 네 동작을 나눠 준다. 파문이 같이 번지는 것은 눈이 오른쪽 레일의 답에 가 있을
  때를 위한 것이다 — 크기 변화만으로는 시야 가장자리에서 안 잡힌다. 끝나도 버튼을 지우지
  않고 **폭을 접어서** 내보낸다: 한 프레임에 없애면 팝으로 읽힌다.
...
- **화면에 들어와야 시작한다.** 히어로를 읽는 동안 혼자 끝나 있으면 아무도 못 본다.
  한 번만 돈다.
- **다 돌면 스스로 처음으로 돌아간다**(1.5초 쉬고). 버튼을 두지 않는다 — 스크롤로 지나쳐
  놓친 사람이 알아서 다시 보게 된다. **손댄 사람에게도 돈다.** 한때 안 돌렸는데, 아무거나
  한 번 눌렀다는 이유로 화면이 그 자리에 굳어 다시 볼 방법이 없었다. 돌 때 고정과 손수
  보낸 질문까지 되돌린다 — 탭이 못 박힌 채로 다시 돌면 대본이 옮기려는 자리마다 막힌다.
- **바뀌는 순간마다 움직인다.** 값만 갈리면 「원래 그랬던 것」으로 읽혀서 무엇이 달라졌는지
  눈이 못 찾는다. 셋에 각각 다른 결을 준다 — 사건 카드는 크림이 한 번 들었다 빠지고
  (`lp-fresh`, 1.6초), 대화는 왕복 단위로 짧게 들며(`lp-rise` 0.24초) 흐르는 답에는 커서가
  붙고, 종료 순간의 화자 칩은 **위에서부터 0.07초씩 밀려** 들어온다. 여덟이 한 프레임에
  붙으면 방금 갈렸다는 것이 안 보인다.
- **커서는 붉지 않다.** 앱의 partial 행 커서는 1px muted고, 붉은 것은 시각 칸의 점이다
  (`transcript-view.tsx`). 커서까지 붉으면 딱지가 둘이 된다.
- **화자는 회의가 끝난 뒤에 붙고, 붙는 것은 이름이 아니라 「화자 A」다.** 앱은 화자 매핑이
  `MAPPED`가 돼야 칩을 그리고(`transcript-view.tsx`의 `diarized`), 사람과 잇는 것은 그다음에
  **사용자가 한다**(`speaker-identity.ts` — 연결 안 됐으면 `화자 A`). 칩 안 글자도 이름의 첫
  자가 아니라 라벨이다: 이름이 없으면 전부 「화」가 되어 얼굴이 서로를 못 가린다. 파스텔은
  **바탕으로만** 쓰고, 뒤의 회색 점은 「아직 확인하지 않은 화자」다 — 눈에 띄어야 이름을
  붙일 이유가 생긴다. 기록 중에는 시각과 본문뿐이고, 받아 적는 중인 줄은 **시각 자리에
  붉은 점과 「받아 적는 중」**이 선다(앱의 partial 행). 실측: 기록 중 6줄·화자 0 → 종료 뒤
  8줄·화자 A~D.
...
칩과 묶음 접기, 예시 질문이 다 동작한다 — 그림만 보여 주는 것보다 앱이 어떤지가 훨씬 빨리
전해진다. `role="tablist"`와 방향키 이동(roving tabIndex)까지 앱과 같게 둔다: 버튼처럼
생겼는데 키보드로 못 쓰면 눌러 보라고 해 놓고 못 누르게 막는 셈이다.
...
| 시안 | 실제 (`note-panel.tsx` · `context-rail.tsx` · `note-archive.tsx` · `note-details.tsx`) |
| --- | --- |
| 정보 탭이 「회의 정보」 표 하나 | 표 **위에 편집 영역**이 있다 — 제목 입력 · 참석자(아바타 + 「참여자 선택」) · 「변경 저장」. 카드는 없고, 편집만 컨트롤 테두리를 갖고 읽기는 키/값 표로 눕는다 |
| 표의 줄마다 밑줄 | `Fact`에는 **테두리가 없다**. 대신 회의의 사실(진행자 · 누적 기록 시간 · 공유 범위)과 문서의 이력(생성 · 최종 수정) 사이에 **선이 하나** 있다 |
| 값이 라벨만 | 진행자 앞에 아바타가 서고, 뒤에 「· 기록 제어 권한」이 muted로 붙는다. 누적 기록 시간에는 「· 종료된 구간만 합산」이 붙는다 |
...
FAQ의 답은 **접혀도 DOM에 남긴다.** 지우면 `aria-controls`가 없는 id를 가리킨다. **DOM에
남기는 것만으로는 페이지 내 찾기(Ctrl+F)가 못 찾는다** — 그냥 `hidden`은 `display:none`이라
찾기 대상에서 통째로 빠진다. 값이 `until-found`여야 찾히고, 찾히면 브라우저가 스스로
펼친다. react가 `hidden`을 불리언으로 못박아 둬서 JSX로는 못 넣고, 붙은 뒤에 올려 쓴다
(`faq.tsx`의 `Answer`). 본문 건너뛰기 링크는
...
**좁은 화면(`< sm`)에서는 탭이 둘째 줄(`44`)로 내려간다 (2026-09-14).** 390 폭 한 줄에 `←` · 창 제어 · 상태 칩 · 탭 셋 · `⋯`이 다 서니 줄어들 수 있는 것이 제목뿐이라 **제목이 0px** 가 되어 어느 회의인지 안 보였다. 첫 줄은 이동 · 상태 · 제목 · 메뉴, 둘째 줄은 탭이다. 줄 수가 탭과 상관없이 늘 둘이라 「탭은 제자리」는 그대로이고, 812×375 landscape는 `sm` 이상이라 한 줄 그대로다. 테스트는 높이 클래스 대신 `data-slot="note-top-bar"`로 바를 찾는다.
...
### 회의 뒤 검토 화면 (Claude Design 캔버스 · 2026-09-14)

요약 탭은 흐름 상태로 갈린다 — 분석이 도는 동안은 단계 표시와 스피너(`flow-notice`), 검토본이 서면 검토 화면(`review-board`), 지난 노트는 구형 요약을 읽기만 한다. 한 회의에서 항목이 백 개 가까이 나오는 크기를 기준으로 짰다.

| 무엇 | 규칙 |
| --- | --- |
| 섹션 | 왼쪽 `100px` 칸에 제목과 개수, 오른쪽에 내용(`section-block`). 결정 · 할 일 · 이슈·질문 · 참고 순서이고, 앞의 다섯 줄만 서고 나머지는 「N개 더」로 접힌다. 참고는 개수만 보이고 통째로 접힌다 |
| 주제 | 개요 아래 **주제 목차**다 — 번호 · 제목 · 한 줄 서술 · 항목 수가 한 줄씩 서고(앞의 셋, 나머지는 「주제 N개 더」), 누르면 그 주제만 거르며 섹션 위에 「주제 02 · 제목 · 전체 보기」가 붙는다. 칩으로 줄였더니 제목이 잘리고 번호만 남아 무엇이 무엇인지 안 읽혔다 |
| 줄 | 할 일과 이슈 · 질문은 `내용 · 담당 128 · 기한 108 · 화살 14` 열이고 칸을 눌러 바로 고친다. 이 둘에는 주제 칸을 두지 않는다 — 담당 · 기한과 함께 서면 내용이 가려졌다. 다른 섹션은 `내용 · 주제 132 · 화살 14` 이고 주제 칸은 번호와 짧은 제목(좁은 화면은 번호만)이다. 요약이 풀렸다고 짚은 이슈 · 질문(`outline.issues[].resolvedByItemIds`)은 취소선과 담당 · 기한 자리의 「해결됨」으로 선다. 펼치면 제안 · 수정 기록 카드 · 관련 항목이 선다 |
| 제안 선택 | 두 칸 토글(`choice-toggle`). 제안은 줄을 펼치지 않아도 줄 바로 아래에 선다 — 사람이 골라야 하는 것이라 접어 두지 않는다. **바꾸는 쪽(반영 · 끝내기)은 초록 + 체크, 유지는 먹색**이다. 빨강은 오류 색이라 고르는 값에 쓰지 않는다. 고른 값은 검토 항목 수정(`decisions`)으로 저장해 새로고침하거나 다른 참석자가 열어도 같게 보이고, 대체 제안에는 끝낼 이전 결정의 내용 · 출처 회의 · 확정일이 선다. 확정은 저장된 끝내기만 끝낸다 |
| 역할 색 | 결정 `#2a78d6` · 할 일 `#1baf7a` · 미해결 `#eb6834` · 참고 `#a8a29e`. 색각 이상 검증을 거친 네 색이고 `role-dot.tsx` 한 곳에 있다. 색 옆에는 늘 이름이 선다 |
| 그래프 보기 | 개요 문장 → 범례(먹색 원 주제 · 속 빈 원 안건 · 역할 넷 / 실선 관계 · 점선 다른 주제에도 속함) → 판. 판은 `react-force-graph-2d` 캔버스의 힘 배치다 — 주제는 이름이 늘 붙는 먹색 허브 점이고 항목은 제 허브에 가장 옅은 선으로 매달린다. 점 크기는 연결 수, 항목 이름은 확대하거나 가리킬 때만 선다. 캔버스는 읽기 프로그램이 못 읽어 같은 항목을 보이지 않는 버튼 목록으로 둔다. 안내 문구는 판 안 왼쪽 아래(좁은 화면은 판 아래)에 둔다 |
| 수정 기록 | 최근 단계가 위. 단계를 펼치면 인용 줄과 앞뒤 한 줄이 서고, 인용 줄은 `--el-highlight` 형광 밑줄, 뒷받침이 아닌 역할만 테두리 배지(반대 의견 · 조건 · 취소 · 언급)를 단다 |
| 카드 | 요약 보기에서 펼친 줄의 수정 기록 카드와 그래프의 상세 패널은 옅은 그림자(`shadow-e2`), 그 패널 안의 카드는 hairline 만. 붙박이 확정 줄은 위로 옅은 그림자 |
| 복사 | 개요 · 주제 · 네 섹션마다 제목 칸에 `복사`(`CopyMarkdownButton`, 전사 · 요약과 같은 버튼)가 선다. 넓은 화면은 개수 아래, 좁은 화면은 제목 줄 오른쪽 끝. 형태는 `## 섹션` + 불릿이고 할 일은 `— 담당 · 기한`, 여러 종류가 섞인 섹션은 `**질문**` 처럼 종류를 앞에 적는다. **뺀 항목은 싣지 않는다**(확정되지 않을 줄이 남에게 가면 안 된다). 주제로 거른 채 복사하면 제목에 `— 주제 제목`을 붙인다. 포함된 항목이 없으면 숨기지 않고 비활성이다 — 자리가 흔들리지 않게 |

**움직임.** 층이 아니라 자리의 전이지만 제품 면과 같은 200ms `ease-out` 을 쓰고, `motion-reduce` 에서는 바로 바뀐다.

| 무엇 | 어떻게 |
| --- | --- |
| 줄 · 단계 펼침 | `Collapse` — grid 행 `0fr → 1fr` + 투명도. 높이를 재지 않는다. 처음 펼칠 때 내용을 만들고(lazy) 접어도 둔다 |
| 요약 ↔ 그래프 | `SegmentedControl` 의 흰 판이 `translateX` 로 미끄러지고, 보기는 `fade-in` 한 번 |
| 새 항목 · 확정 줄 숫자 | 새로 선 줄만 `fade-in + slide-in-from-top-1`, 숫자는 바뀔 때만 `zoom-in-95` |
| 그래프 | 배치는 열 때 220틱 돌고 멈춘 뒤 판에 맞춘다(첫 좌표를 ID 로 정해 같은 검토본은 같은 모양). 가리키면 그 점과 이웃만 밝고 나머지는 흐려진다. 점은 끌 수 있고, 밖에 있는 노드를 고르면 판이 300ms 로 그 자리로 옮겨 간다 |

**좁은 화면(`< sm`).** 고정 폭 열은 내용 아래 둘째 줄로 내려간다 — 칸을 감싼 상자가 넓은 화면에서 `sm:contents` 로 사라져 표의 열 순서가 그대로다. 표 머리는 숨긴다. 담당 · 기한 칸은 좁은 화면에서 `h-9`, 넓은 화면에서 `h-7` 이다. 섹션 제목 칸은 위아래로 쌓인다.
