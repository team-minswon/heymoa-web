...
- **화면에 들어와야 시작한다.** 히어로를 읽는 동안 혼자 끝나 있으면 아무도 못 본다.
  한 번만 돈다.
- **다 돌면 창 아래에 「처음부터 다시 보기」가 선다.** 스크롤로 지나쳐 놓친 사람에게 남는
  유일한 길이다. 누르면 고정해 둔 탭과 손으로 보낸 질문까지 되돌린다 — 탭이 못 박힌 채로
  다시 돌면 대본이 옮기려는 자리마다 막혀 반쪽만 보인다. **자리는 미리 잡아 둔다**(h-8):
  끝날 때 버튼이 끼어들면 아래 밴드가 통째로 밀린다. 모션을 줄인 사람에게는 안 낸다 —
  돌릴 것이 없다.
