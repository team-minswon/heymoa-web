...
**도착한 줄은 형광펜으로 짚고, 여백은 건드리지 않는다.** 회색 틴트(`--el-surface-strong`)는 hover·열림 표시가 이미 쓰는 값이라 2.4초만 사는 이 표시가 구별되지 않았다 — `--el-highlight`(형광 노랑)로 바꿨다. 그리고 예전에는 `px-3`이 함께 붙어서 **도착한 줄만 글자가 12px 밀리고 아래 hairline이 24px 짧아졌다.** 찾아간 자리가 도착과 동시에 움직이는 셈이다. 여백으로 강조하려면 안 짚힌 블록도 같은 여백을 늘 갖고 있어야 하는데 그러면 전사 본문 전체의 정렬이 바뀐다 — 색 하나로 끝내는 편이 싸다.
