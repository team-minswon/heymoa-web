...
| 그래프 보기 | 개요 문장 → 범례(속 빈 원 안건 · 역할 넷 / 실선 관계 · 점선 다른 주제에도 속함) → 판. 안건 노드는 속이 빈 원이고, 안내 문구는 판 안 왼쪽 아래(좁은 화면은 판 아래)에 둔다 |
...
| 그래프 | 호버 흐림 · 선택 고리는 CSS 전이. 밖에 있는 노드를 고르면 viewBox 가 300ms 로 옮겨 가는데, viewBox 는 CSS 로 못 옮겨 여기만 프레임 단위 JS 다 |
