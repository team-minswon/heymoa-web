...
- **상단바** — 아트보드는 캔버스에 붙은 평평한 바(높이 76)로 그렸지만 실제는 루트 레이아웃의
  떠 있는 알약(`fixed top-4`)이다. 히어로가 `pt-[104px]`(데스크톱 `pt-[136px]`)로 그 몫을
  비운다. 그래서 히어로 밴드만 아트보드보다 43px 높다.
...
| 어디 | 차 | 왜 |
| --- | --- | --- |
| 히어로 (양쪽) | +43 / +46 | 떠 있는 상단바 몫. 의도한 차이다 |
| 제품 샷 (1440) | −18 | 레일 항목은 아트보드와 1px도 안 틀리는데 그룹 사이에서 벌어진다 |
| 왜 만드나 (390) | −15 | 상자를 데스크톱 판(크림)으로 통일한 몫 |
| 사용 흐름 (390) | +18 | 모바일 아트보드는 5번 걸음의 「개요」 문장을 줄여 적는다. 폭에 따라 문장을 바꾸지 않았다 |
| CTA (1440) | +4 | 15px 한 줄의 line box가 1px 다르다. 폰트 메트릭 차이라 더 좁힐 수 없다 |

나머지 밴드는 1440에서 ±2px, 390에서 ±9px 안에 든다. 문서 전체는 8199 / 8248(1440),
11015 / 11022(390)이다.
