...
**제품 샷은 눌린다.** 정보·전사·요약과 실시간 정리·내 에이전트가 진짜 탭이고, 사건 범위
칩과 묶음 접기도 동작한다 — 그림만 보여 주는 것보다 앱이 어떤지가 훨씬 빨리 전해진다.
`role="tablist"`와 방향키 이동(roving tabIndex)까지 앱과 같게 둔다: 버튼처럼 생겼는데
키보드로 못 쓰면 눌러 보라고 해 놓고 못 누르게 막는 셈이다. 탭 정거장은 둘만 늘어난다
(목록당 하나).

반대로 **흉내만 내는 것은 여전히 `<span>`이다** — 뒤로·전체화면·노트 메뉴·복사. 눌러도 할
일이 없는 것을 버튼으로 두면 탭 순회에 빈 정거장이 늘 뿐이다.

**「내 에이전트」에서는 실제로 묻는다.** 준비된 질문 셋을 칩으로 두고, 누르면 답이 글자
단위로 흐른 뒤 참고한 회의록이 붙는다. 대화는 쌓이고 바닥에 붙어 따라간다.
