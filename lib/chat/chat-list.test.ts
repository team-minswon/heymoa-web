...
const row = (chatId: string, turnId: string | null, status = "IN_PROGRESS") => ({
  chatId,
...
describe("배지는 두 출처를 턴 id 로 맞춘다", () => {
  ...
  it("목록엔 없는데 스트림이 흐르면 스트림이 이긴다", () => {
    expect(runningLabel(row("c1", null), open("c1", "t1", "streaming"))).toBe("진행 중");
  });

  it("다른 턴 id 면 목록이 유일한 소식이다", () => {
    expect(runningLabel(row("c1", "t2"), open("c1", "t1", "done"))).toBe("진행 중");
  });

  it("승인 대기는 다른 라벨이고 양쪽 출처가 같은 말을 한다", () => {
    expect(runningLabel(row("c1", "t1"), open("c1", "t1", "awaiting_approval"))).toBe(
      "승인 대기"
    );
    expect(runningLabel(row("c2", "t9", "WAITING_APPROVAL"), open("c1", null, "idle"))).toBe(
      "승인 대기"
    );
  });

  it("아무 대화도 안 열려 있으면 목록이 전부다", () => {
    expect(runningLabel(row("c1", "t1"), open(null, null, "idle"))).toBe("진행 중");
...
describe("시간으로 묶는다", () => {
  ...
  it("없는 묶음은 안 그린다", () => {
    expect(
      groupChatsByRecency([at("2026-08-25T09:00:00Z")], NOW, TZ).map((g) => g.label)
    ).toEqual(["오늘"]);
  });
