...
describe("v1 범위 — 기각 표면은 없다", () => {
  ...
  it("계약 스키마가 기각 필드를 알지 못한다", () => {
    ...
    const parsed = proposalSnapshotSchema.parse({
      proposals: [],
      runs: [],
      rejectedProposals: [
        { proposalId: "0HZX2K7M9Q4A1", reason: "없는 내용" },
      ],
    });

    expect(parsed).not.toHaveProperty("rejectedProposals");
    expect(Object.keys(parsed).sort()).toEqual(["proposals", "runs"]);
  });

  it("후보 head 의 필드 집합이 합의된 것뿐이다", () => {
    // 필드가 늘면 여기가 먼저 깨진다 — 계약 변경을 사람이 한 번 보게 하는 자리다.
    expect(Object.keys(proposalHeadSchema.shape).sort()).toEqual([
  ...
  it("기각 조회를 위한 생성 훅이 없다", () => {
    const generated = read(
      "lib/api/generated/proposals/proposals.ts"
    );
    ...
    expect([...hooks].sort()).toEqual([
      "useGetProposalRevisions",
      "useGetProposalRevisionsSuspense",
      "useGetProposals",
      "useGetProposalsSuspense",
    ]);
  });
});
