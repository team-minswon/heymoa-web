import { describe, expect, it } from "vitest";

import type { RunRange } from "@/lib/notes/proposals/contract";
import { withCoverageRows } from "@/lib/notes/proposals/timeline";
import type { TranscriptRow } from "@/lib/transcription/presentation";

function segment(startedAtMs: number, segmentId: string): TranscriptRow {
  return {
    type: "segment",
    startedAtMs,
    segment: {
      segmentId,
      sequence: startedAtMs / 10_000,
      text: "발화",
      startedAtMs,
      endedAtMs: startedAtMs + 5_000,
    },
  };
}

function range(
  fromSequence: number,
  toSequence: number,
  over: Partial<RunRange> = {}
): RunRange {
  return {
    runId: `run-${fromSequence}`,
    applyStatus: "APPLIED",
    fromSequence,
    toSequence,
    fromStartedAtMs: fromSequence * 10_000,
    toEndedAtMs: toSequence * 10_000,
    rawDeltaSaturated: false,
    semanticUnitSaturated: false,
    appliedAt: "2026-08-24T01:00:00.000Z",
    ...over,
  };
}

describe("withCoverageRows", () => {
  it("범위가 이어지면 아무것도 얹지 않는다", () => {
    const rows = [segment(10_000, "a"), segment(20_000, "b")];
    expect(withCoverageRows(rows, [range(1, 10), range(11, 20)])).toEqual(rows);
  });

  it("범위 구멍이 회의 축의 제자리에 선다", () => {
    const rows = [segment(50_000, "a"), segment(200_000, "b")];
    const merged = withCoverageRows(rows, [range(1, 10), range(16, 25)]);

    expect(merged.map((row) => row.type)).toEqual([
      "segment", // 50_000
      "coverage-gap", // 100_000 – 160_000
      "segment", // 200_000
    ]);
  });

  it("포화·부분 반영 범위는 스크립트에 안내 행을 얹지 않는다", () => {
    const runs = [
      range(1, 10, { rawDeltaSaturated: true }),
      range(11, 20, { semanticUnitSaturated: true }),
      range(21, 30, { applyStatus: "PARTIAL_RECORDED" }),
    ];

    expect(withCoverageRows([], runs)).toEqual([]);
  });

  it("양끝을 넘기면 선두·후미 공백도 선다 — 종료된 전사의 거절된 첫/마지막 실행", () => {
    const rows = [segment(10_000, "0K1"), segment(100_000, "0K2")];
    const edges = {
      fromSequence: 1,
      toSequence: 10,
      fromStartedAtMs: 10_000,
      toEndedAtMs: 105_000,
    };
    const merged = withCoverageRows(rows, [range(3, 7)], edges);
    const gaps = merged.filter((row) => row.type === "coverage-gap");
    expect(gaps.map((row) => [row.gap.fromSequence, row.gap.toSequence])).toEqual([
      [1, 2],
      [8, 10],
    ]);
  });

  it("범위가 하나도 없으면 양끝을 넘겨도 안 만든다 — 분류가 안 돈 회의와 구분이 안 된다", () => {
    const rows = [segment(10_000, "0K1")];
    const edges = {
      fromSequence: 1,
      toSequence: 10,
      fromStartedAtMs: 10_000,
      toEndedAtMs: 105_000,
    };
    expect(withCoverageRows(rows, [], edges)).toEqual(rows);
  });

  it("원본 배열을 건드리지 않는다 — 복사에 나가는 전사와 화면용이 갈린다", () => {
    // 마크다운 복사는 회의록이라 분류 커버리지 같은 화면 주석이 섞이면 안 된다.
    // `transcript-view` 가 `rows`(전사) 와 `renderRows`(전사+커버리지) 를 나눠 쓰는 근거다.
    const rows = [segment(50_000, "a")];
    const before = [...rows];
    withCoverageRows(rows, [range(1, 10)]);
    expect(rows).toEqual(before);
    expect(rows).toHaveLength(1);
  });
});
