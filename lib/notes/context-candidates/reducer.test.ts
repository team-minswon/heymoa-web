...
describe("context candidate reducer", () => {
  ...
  it("snapshot은 자기보다 새 것을 되돌리지 않는다 — WS로 먼저 온 후보와 병합한다", () => {
    ...
    const live = apply([
      changed(head({ candidateId: "0HZX2K7M9Q4A1" })),
      changed(
        head({ candidateId: "0HZX2K7M9Q4A1", revision: 2, operation: "AMEND" }),
        "0HZX2K7M9Q4E2"
      ),
    ]);
    const settled = reduceContextEvent(live, {
      ...
      candidates: [
        ...
        head({ candidateId: "0HZX2K7M9Q4A2", createdSequence: 5 }),
      ],
      appliedRanges: [range()],
    });

    expect(
      selectCards(settled).map((c) => [c.candidateId, c.revision])
    ).toEqual([
      ["0HZX2K7M9Q4A2", 1],
      ["0HZX2K7M9Q4A1", 2],
    ]);
