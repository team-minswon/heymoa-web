...
export function reduceContextEvent(
  ...
  switch (event.type) {
    ...
    case "snapshot": {
       ...
       * revision이 큰 쪽을, 범위는 snapshot에 없는 runKey를 보존하며 합친다
       * (rule `architecture`: reconcile 지점은 provider 한 곳, 확정 데이터로 수렴).
       */
      const candidates = byId(event.candidates);
      for (const current of Object.values(state.candidates)) {
        const incoming = candidates[current.candidateId];
        if (!incoming || incoming.revision < current.revision) {
          candidates[current.candidateId] = current;
        }
      }
      const snapshotRunKeys = new Set(
        event.appliedRanges.map((range) => range.runKey)
      );
      const ranges = sortRanges([
        ...event.appliedRanges,
        ...state.appliedRanges.filter(
          (range) => !snapshotRunKeys.has(range.runKey)
        ),
      ]);
