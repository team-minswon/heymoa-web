 ...
 *
 * 1. **`runKey`는 확정 계약에서 TSID 13자다.** draft 시절에는 opaque `^[A-Za-z0-9_-]{1,64}$`
 *    였는데 확정 미러의 `RunKey`가 TSID로 좁혔다 — URL path에 나가는 외부 식별자이기
...
/**
 * 멱등키. **확정 계약(RunKey)이 TSID 13자로 좁혔다** — draft 시절의 opaque 1~64자가
 * 아니다. 미러(`openapi3.yml`·`asyncapi.yml`)의 `^[0-9A-HJKMNP-TV-Z]{13}$`를 그대로 쓴다.
 */
const runKeySchema = tsidSchema;
