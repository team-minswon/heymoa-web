...
| `POST /v1/auth/logout`  | `useLogout`         | 툴바 계정 메뉴                                  | 성공 시 `/`로 하드 내비게이션     | 기존 화면(v1)         |
...
**표시 방식은 rule [`error-loading`](../harness/v001-2026-07-26/rules/error-loading.md)을 따른다.** 요약하면 mutation 실패는
...
rule [`error-loading`](../harness/v001-2026-07-26/rules/error-loading.md)이 규칙이고, 여기서는 이 계약에 적용한 결과만 적는다.
