> **이전 세대 · 참조용.** 현 세대 디자인은 `design-decisions.md`다. 이 파일은 v4 프레임의
> 결정 이력이며 v5로 대체됐다 — v5가 무엇을 왜 바꿨는지는 `mvp.pen`의 v5 SPEC 노트와
> `design-decisions.md`를 본다. v4 행은 2026-07-27에 정리됐다 — 살아남은 30장은 v1 열에 섞여 있다.
...
`mvp.pen`은 git 밖이고, 이 작업의 변경은 **pencil 편집기 세션에만 있다** — 디스크 파일은
작업 전 상태 그대로다(SHA-256 동일). pencil MCP에는 저장 도구가 없어 앱에서 직접 저장해야 한다.

그래서 신규 25개와 수정 8개를 **`docs/design/frames/{프레임ID}.png`로 내보내 git에 넣었다.**
세션이 유실돼도 무엇을 그렸는지가 남는다. 아래 표의 프레임 ID가 곧 파일 이름이다.

`.pen`이 저장되면 이 이미지는 그 시점의 기록으로 남고, 이후 변경은 다시 내보내 갱신한다.

> **2026-07-27 — 이 경로는 더 이상 없다 (APP-213 ③).**
> 캔버스가 v1 한 세대로 통합되면서 `docs/design/frames/`의 ID 파일명 PNG는 전부 정리됐다.
> v4 프레임 중 **살아남은 30장은 `docs/design/frames/v1/`에 프레임 이름으로** 들어 있고
> (`chat_approval-expired.png` 식), 나머지는 v5가 다시 그렸거나 폐기돼 지웠다.
> 무엇이 남고 무엇이 사라졌는지는 [`design-decisions.md`](design-decisions.md)의 「v1 통합」을 본다.
> **이 문서는 v4 시점의 기록이므로 본문은 고치지 않는다.**
...
- **초대 수락 후 "그 워크스페이스가 목록에 나타난다"** 상태의 프레임이 아직 없다. (기본 워크스페이스는 APP-401에서 사라졌다 — 그쪽 절반은 더 이상 그릴 것이 없다.) 이 상태는 사이드바 워크스페이스 스위처에서 드러나므로 이 행이 아니라 워크스페이스 셸 행에서 다뤄야 한다.
