...
- **노트 목록 상태 배지** — `LHXhy` 등에 녹음 중/요약 완료 배지가 있으나 목록 계약에 `meetingStatus`가
  없다. [APP-159](https://linear.app/minswon/issue/APP-159)(계약 확장)에 블록. 계약이 먼저다.
...
v5는 계약을 바꾸지 않는다. 프레임 데이터는 원칙적으로 `openapi3.yml`로 채운다.
계약에 없는 데이터가 필요하면 화면을 밀지 말고 계약 이슈를 연다(APP-145에서 계약에 없는 알림 타입을
그렸다가 삭제한 전례). 노트 목록 배지가 그 사례이고 [APP-159](https://linear.app/minswon/issue/APP-159)로 분리했다.
